...
SELECT id, product_id, qty, order_id, created_at FROM order_items ORDER BY order_id LIMIT $1 OFFSET $2
`
...
func (q *Queries) ListOrderItemByOrderId(ctx context.Context, arg ListOrderItemByOrderIdParams) ([]OrderItem, error) {
	rows, err := q.db.QueryContext(ctx, listOrderItemByOrderId, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
